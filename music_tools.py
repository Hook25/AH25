...
import os
import random
from utility import sys_exec
from utility import get_tmp_name, panic

#TODO: make this module parametric
CONCAT_COMMAND = "ffmpeg -f concat -safe 0 -i %s -c copy -hide_banner -loglevel error -y %s"
SET_MUSIC_COMMAND = "ffmpeg -i %s -i %s -c:v copy -map 0:v:0 -map 1:a:0 -hide_banner -loglevel error -shortest -y %s"
...
def put_music(input, music_path, clips, tmp_folder):
  ...
  output_name_file = get_tmp_name(".ts", "tmp_", tmp_folder)
